...
const ReviewCard = ({ review }: { review: Review }) => (
  <div className="luxury-card rounded-xl p-6 flex flex-col gap-4 h-full">
    ...
    <div className="flex items-center gap-3 pt-2 border-t border-border mt-auto">
      <div className="w-9 h-9 rounded-full bg-gradient-primary flex items-center justify-center shrink-0 overflow-hidden">
        {review.photoUri ? (
          ...
          <span className="font-body text-sm font-bold text-primary-foreground">{review.avatar}</span>
        )}
      </div>
      <div>
    ...
    const fetchReviews = async () => {
      try {
        ...
        if (data?.reviews?.length > 0) {
          ...
          const rotated = [...allReviews.slice(offset), ...allReviews.slice(0, offset)];
          setReviews(rotated);
      ...
      <div className="container mx-auto px-6">
        <div className="text-center mb-10">
          <div className="flex items-center justify-center gap-3 mb-3">
            <div className="flex items-center gap-1">
              {[1, 2, 3, 4, 5].map((i) => (
                <Star key={i} size={18} className={i <= Math.round(overallRating) ? "fill-gold text-gold" : "text-muted-foreground"} />
              ))}
            </div>
            <span className="font-body text-xl font-semibold text-foreground">{overallRating.toFixed(1)}</span>
              ...
              {Array.from({ length: count }).map((_, i) => (
                <div
                  ...
                  className={`h-1.5 rounded-full transition-all duration-300 ${
                    i === current ? "w-6 bg-gold" : "w-1.5 bg-muted-foreground/30"
                  }`}
                />
              ))}
            </div>
